...
import type { WatchlistItem, WatchlistAlert } from '@/types/watchlist'
import type { SharedInsight } from '@/types/community'
...
// ---------------------------------------------------------------------------
// Mock calendar events — spread across next 30 days
// ---------------------------------------------------------------------------

export const MOCK_CALENDAR_EVENTS: CalendarEvent[] = [
  {
    id: 'cal-1',
    title: 'ARB Governance Vote — Treasury Allocation',
    ...
    asset: 'ARB',
    event_date: new Date(Date.now() + 5 * 24 * 3600000).toISOString(),
    end_date: null,
    impact: 'medium',
    source: 'Arbitrum DAO',
    source_url: 'https://forum.arbitrum.foundation',
    pelican_context: 'Explain this ARB Governance Vote — Treasury Allocation event. What historically happens to ARB around this type of governance vote? How should the user prepare? What derivatives positioning should they watch?',
    metadata: null,
    created_at: new Date().toISOString(),
  },
  {
    id: 'cal-2',
    title: 'ETH Dencun Upgrade Anniversary',
    description: 'One-year anniversary of Ethereum\'s Dencun upgrade that introduced EIP-4844 (proto-danksharding). L2 transaction costs dropped 95%. Market typically rallies on upgrade anniversaries as they remind traders of fundamental progress.',
    event_type: 'upgrade',
    asset: 'ETH',
    event_date: new Date(Date.now() + 6 * 24 * 3600000).toISOString(),
    end_date: null,
    impact: 'low',
    source: 'Ethereum Foundation',
    source_url: 'https://ethereum.org',
    pelican_context: 'Explain this ETH Dencun Upgrade Anniversary event. What historically happens to ETH around upgrade anniversaries? How should the user prepare? What has changed since Dencun launched?',
    metadata: null,
    created_at: new Date().toISOString(),
  },
  {
    id: 'cal-3',
    title: 'BTC Options Expiry — $4.2B Notional',
    ...
    asset: 'BTC',
    event_date: new Date(Date.now() + 7 * 24 * 3600000).toISOString(),
    end_date: null,
    impact: 'high',
    source: 'Deribit',
    source_url: 'https://www.deribit.com',
    pelican_context: 'Explain this BTC Options Expiry event. What historically happens to BTC around large options expirations? How should the user prepare their portfolio? What derivatives positioning should they watch? Any TradFi parallels with equity options expiration?',
    metadata: { notional: 4200000000, max_pain: 82000 },
    created_at: new Date().toISOString(),
  },
  {
    id: 'cal-4',
    title: 'SOL Token Unlock — $400M Vesting Release',
    description: '$400M worth of SOL tokens unlock from early investor and team vesting schedules. Token unlocks are like insider lock-up expirations in IPOs — they increase circulating supply and can create selling pressure if holders choose to liquidate.',
    event_type: 'token-unlock',
    asset: 'SOL',
    event_date: new Date(Date.now() + 8 * 24 * 3600000).toISOString(),
    end_date: null,
    impact: 'high',
    source: 'Token Unlocks',
    source_url: 'https://token.unlocks.app',
    pelican_context: 'Explain this SOL Token Unlock — $400M event. What historically happens to SOL around token unlocks? How should the user prepare? What derivatives positioning should they watch? How is this similar to IPO lock-up expirations?',
    metadata: { unlock_value_usd: 400000000 },
    created_at: new Date().toISOString(),
  },
  {
    id: 'cal-5',
    title: 'Fed FOMC Meeting — Rate Decision',
    description: 'Federal Reserve FOMC meeting with rate decision announcement at 2:00 PM ET. Markets currently pricing 92% probability of hold. Crypto historically sees 3-5% moves in the 2 hours after FOMC announcements, with direction depending on dot plot surprises.',
    event_type: 'fed-meeting',
    asset: null,
    event_date: new Date(Date.now() + 12 * 24 * 3600000).toISOString(),
    end_date: null,
    impact: 'high',
    source: 'Federal Reserve',
    source_url: 'https://www.federalreserve.gov',
    pelican_context: 'Explain this Fed FOMC Meeting event. What historically happens to crypto around FOMC meetings? How should the user prepare their portfolio? What derivatives positioning should they watch? How do crypto markets compare to equity markets in their FOMC reaction?',
    metadata: { rate_probability_hold: 0.92 },
    created_at: new Date().toISOString(),
  },
  {
    id: 'cal-6',
    title: 'Coinbase Q4 Earnings Report',
    description: 'Coinbase Global (COIN) reports Q4 2025 earnings after market close. Strong earnings typically boost crypto sentiment broadly. Revenue expectations: $1.8B. EPS estimate: $2.45.',
    event_type: 'earnings',
    asset: null,
    event_date: new Date(Date.now() + 13 * 24 * 3600000).toISOString(),
    end_date: null,
    impact: 'medium',
    source: 'Nasdaq',
    source_url: 'https://www.nasdaq.com/market-activity/stocks/coin',
    pelican_context: 'Explain this Coinbase Q4 Earnings event. What historically happens to crypto around Coinbase earnings? How do exchange earnings reflect on the broader crypto market? What should the user watch for?',
    metadata: { revenue_estimate: 1800000000, eps_estimate: 2.45 },
    created_at: new Date().toISOString(),
  },
  {
    id: 'cal-7',
    title: 'Crypto Regulatory Hearing — US Senate Banking Committee',
    description: 'US Senate Banking Committee hearing on crypto market structure legislation. Key topics: stablecoin regulation, exchange licensing, and DeFi classification. Regulatory clarity is generally bullish for institutional adoption.',
    event_type: 'other',
    asset: null,
    event_date: new Date(Date.now() + 14 * 24 * 3600000).toISOString(),
    end_date: null,
    impact: 'high',
    source: 'US Senate',
    source_url: 'https://www.banking.senate.gov',
    pelican_context: 'Explain this Crypto Regulatory Hearing event. What historically happens to crypto around regulatory hearings? How should the user prepare? What specific regulatory outcomes would be bullish vs bearish?',
    metadata: null,
    created_at: new Date().toISOString(),
  },
  {
    id: 'cal-8',
    title: 'LINK Staking v2 Launch',
    description: 'Chainlink launches Staking v2 with expanded capacity and community staking pools. Previous staking launch drove 15% price appreciation in the week following. Staking reduces circulating supply, creating positive supply dynamics.',
    event_type: 'upgrade',
    asset: 'LINK',
    event_date: new Date(Date.now() + 15 * 24 * 3600000).toISOString(),
    end_date: null,
    impact: 'medium',
    source: 'Chainlink',
    source_url: 'https://chain.link',
    pelican_context: 'Explain this LINK Staking v2 Launch event. What historically happens to LINK around staking events? How does staking affect supply dynamics? How should the user evaluate this for their watchlist?',
    metadata: null,
    created_at: new Date().toISOString(),
  },
  {
    id: 'cal-9',
    title: 'DOT Parachain Lease Expiry Batch #18',
    description: 'Batch of Polkadot parachain leases expire. Released DOT tokens return to staking or liquid markets. Previous batch expirations have had minimal price impact but created short-term volatility.',
    event_type: 'expiration',
    asset: 'DOT',
    event_date: new Date(Date.now() + 18 * 24 * 3600000).toISOString(),
    end_date: null,
    impact: 'medium',
    source: 'Polkadot',
    source_url: 'https://polkadot.network',
    pelican_context: 'Explain this DOT Parachain Lease Expiry event. What historically happens to DOT around lease expirations? How does this affect the supply dynamics? How is this similar to bond maturities in TradFi?',
    metadata: null,
    created_at: new Date().toISOString(),
  },
  {
    id: 'cal-10',
    title: 'MicroStrategy Earnings — BTC Treasury Update',
    description: 'MicroStrategy reports earnings with update on BTC treasury holdings. CEO typically announces new BTC purchases during earnings. MSTR stock is a leveraged BTC proxy — their buying announcements move BTC price.',
    event_type: 'earnings',
    asset: 'BTC',
    event_date: new Date(Date.now() + 18 * 24 * 3600000).toISOString(),
    end_date: null,
    impact: 'medium',
    source: 'Nasdaq',
    source_url: 'https://www.nasdaq.com/market-activity/stocks/mstr',
    pelican_context: 'Explain this MicroStrategy Earnings event. How does MicroStrategy\'s BTC treasury update affect BTC price? What has happened historically after their BTC purchase announcements? How is MSTR a leveraged BTC proxy?',
    metadata: null,
    created_at: new Date().toISOString(),
  },
  {
    id: 'cal-11',
    title: 'UNI v4 Governance Vote',
    description: 'Uniswap governance vote on v4 deployment parameters including hook permissions and fee structure. Uniswap v4 introduces customizable liquidity pools. Major protocol upgrades typically drive 8-15% appreciation.',
    event_type: 'governance',
    asset: 'UNI',
    event_date: new Date(Date.now() + 19 * 24 * 3600000).toISOString(),
    end_date: null,
    impact: 'medium',
    source: 'Uniswap Governance',
    source_url: 'https://gov.uniswap.org',
    pelican_context: 'Explain this UNI v4 Governance Vote event. What historically happens to UNI around governance votes? How does Uniswap v4 change the DeFi landscape? What should the user watch?',
    metadata: null,
    created_at: new Date().toISOString(),
  },
  {
    id: 'cal-12',
    title: 'BTC CME Futures Monthly Expiration',
    description: 'CME Bitcoin futures monthly contracts expire. CME gaps from weekend closes often fill during expiration week. Institutional positioning via CME futures creates predictable volatility patterns familiar to futures traders.',
    event_type: 'expiration',
    asset: 'BTC',
    event_date: new Date(Date.now() + 21 * 24 * 3600000).toISOString(),
    end_date: null,
    impact: 'high',
    source: 'CME Group',
    source_url: 'https://www.cmegroup.com',
    pelican_context: 'Explain this BTC CME Futures Monthly Expiration event. What historically happens to BTC around CME expirations? How do CME gaps influence price action? How is this similar to ES futures expiration? What derivatives positioning should the user watch?',
    metadata: null,
    created_at: new Date().toISOString(),
  },
]
...
// ---------------------------------------------------------------------------
// Mock watchlist items — assets NOT in mock portfolio
// ---------------------------------------------------------------------------
...
export const MOCK_WATCHLIST_ITEMS: WatchlistItem[] = [
  {
    ...
    user_id: 'mock-user',
    asset: 'DOGE',
    notes: 'Watching for meme cycle breakout above $0.20',
    created_at: new Date(Date.now() - 7 * 24 * 3600000).toISOString(),
    current_price: 0.182,
    price_change_24h: 5.4,
    funding_rate: 0.0003,
    sparkline: generateSparkline(0.165, 0.182, 24, 0.015),
  },
  {
    id: 'watch-2',
    user_id: 'mock-user',
    asset: 'UNI',
    notes: 'v4 governance vote coming up',
    created_at: new Date(Date.now() - 5 * 24 * 3600000).toISOString(),
    current_price: 12.45,
    price_change_24h: -2.1,
    funding_rate: 0.0001,
    sparkline: generateSparkline(12.8, 12.45, 24, 0.012),
  },
  {
    id: 'watch-3',
    user_id: 'mock-user',
    ...
    funding_rate: -0.0002,
    sparkline: generateSparkline(124.5, 128.9, 24, 0.01),
  },
  {
    id: 'watch-4',
    user_id: 'mock-user',
    asset: 'ARB',
    notes: 'Governance vote on treasury allocation — watching funding',
    created_at: new Date(Date.now() - 2 * 24 * 3600000).toISOString(),
    current_price: 1.34,
    price_change_24h: 8.7,
    funding_rate: 0.0008,
    sparkline: generateSparkline(1.18, 1.34, 24, 0.02),
  },
  {
    id: 'watch-5',
    user_id: 'mock-user',
    ...
    funding_rate: 0.0002,
    sparkline: generateSparkline(2.72, 2.67, 24, 0.011),
  },
]
...
export const MOCK_WATCHLIST_ALERTS: WatchlistAlert[] = [
  ...
  },
  {
    id: 'walert-2',
    user_id: 'mock-user',
    watchlist_id: 'watch-4',
    alert_type: 'funding-above',
    condition: { rate: 0.0005 },
    enabled: true,
    last_triggered: new Date(Date.now() - 12 * 3600000).toISOString(),
    created_at: new Date(Date.now() - 2 * 24 * 3600000).toISOString(),
  },
]

// ---------------------------------------------------------------------------
// Mock shared insights — previously shared Pelican Portal conversations
// ---------------------------------------------------------------------------

export const MOCK_SHARED_INSIGHTS: SharedInsight[] = [
  {
    id: 'insight-1',
    user_id: 'mock-user',
    username: 'TraderJack',
    question: 'Why is SOL funding rate positive while price is dropping?',
    answer: 'This is a classic divergence between derivatives positioning and spot price action. Positive funding means longs are paying shorts — traders are still bullish despite the price decline. In TradFi terms, think of it like open interest increasing on ES futures while price falls: someone is building a large long position against the trend. Historically, when SOL funding stays elevated (>0.01%) during a 5%+ pullback, price has bounced within 48 hours in 4 of 6 instances over the past year.',
    portal_conversation_id: null,
    shared_to: 'clipboard',
    likes_count: 12,
    created_at: new Date(Date.now() - 6 * 3600000).toISOString(),
  },
  {
    ...
    user_id: 'mock-user',
    username: 'TraderJack',
    question: 'How does the CME gap affect BTC price action?',
    answer: 'CME gaps are unique to crypto because Bitcoin trades 24/7 but CME futures close on weekends. When BTC moves significantly over the weekend, it creates a "gap" between Friday\'s close and Monday\'s open. These gaps fill approximately 77% of the time. For a futures trader, think of it like a gap-and-go pattern on ES at the Monday open — except crypto gaps tend to fill more reliably because there\'s no single dominant exchange setting the "fair" price.',
    portal_conversation_id: null,
    shared_to: 'clipboard',
    likes_count: 8,
  ...
  },
  {
    ...
    user_id: 'mock-user',
    username: 'TraderJack',
    question: 'Explain liquidation cascades — are they like short squeezes?',
    answer: 'Liquidation cascades are the crypto equivalent of a short squeeze amplified by leverage. When overleveraged positions get liquidated, the forced selling (or buying) moves the price further, triggering more liquidations in a chain reaction. Unlike equity short squeezes which unfold over days, crypto liquidation cascades happen in minutes because positions are liquidated automatically by smart contracts. The key metric to watch is aggregate open interest relative to market cap — when OI/MCap ratio exceeds 3%, cascade risk is elevated.',
    portal_conversation_id: null,
    shared_to: 'clipboard',
    likes_count: 15,
