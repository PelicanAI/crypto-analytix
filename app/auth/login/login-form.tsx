...
export default function LoginForm() {
  ...
  async function handleEmailLogin(e: React.FormEvent) {
    ...
    try {
      ...
      // Check terms + onboarding status to route correctly
      ...
      } else if (!userCredits?.onboarding_complete) {
    ...
    <div className="flex min-h-screen items-center justify-center px-4" style={{ backgroundColor: 'var(--bg-base)' }}>
      <div className="w-full max-w-sm">
        ...
        <p className="mt-6 text-center text-sm" style={{ color: 'var(--text-secondary)' }}>
          ...
          <Link href="/auth/signup" className="font-medium" style={{ color: 'var(--accent-hover)' }}>
            Sign up
          </Link>
        </p>
      </div>
